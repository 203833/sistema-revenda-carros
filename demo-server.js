...
let proximoIdCarro = 3;
let proximoIdVenda = 2;
...
const autenticarToken = (req, res, next) => {
  ...
  jwt.verify(token, 'chave-secreta-padrao', (err, usuario) => {
    if (err) {
...
app.get('/api/v1/cars/brand/:brand', (req, res) => {
...
app.get('/api/v1/sales/customer/:customerId', (req, res) => {
  const customerId = parseInt(req.params.customerId);
  const vendasCliente = vendas.filter(v => v.cliente.id === customerId);
...
app.post('/api/v1/sales/customer/:customerId/car/:carId', autenticarToken, (req, res) => {
  const customerId = parseInt(req.params.customerId);
